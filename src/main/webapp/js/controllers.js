...
function MainCtrl($scope,notify,$http) {
    ...
    //业务组
    $scope.businessList = angular.copy(initData.BUSINESS_TYPE);
    $scope.businessList.unshift({text:"全部",value:""});

    //权益列表
    $scope.rewardTypeList = angular.copy(initData.REWARD_TYPE);


    //权益来源类型
    $scope.fromTypeList = angular.copy(initData.FROM_TYPE);

    //积分来源系统
    $scope.fromSystemList = angular.copy(initData.FROM_SYSTEM);

    //物品分类
    $scope.goodsTypeList = angular.copy(initData.GOODS_TYPE);

    //会员活动类型
    $scope.actTypeList = angular.copy(initData.ACT_TYPE);

    //流水下方方式
    $scope.grantTypeList = angular.copy(initData.GRANT_TYPE);

    //触发类型
    $scope.triggerTypeList = angular.copy(initData.TRIGGER_TYPE);

    //频率类型
  $scope.frequencyTypeList = angular.copy(initData.FREQUENCY_TYPE)

    //活动有效期类型
    $scope.expriryDateTypeList = angular.copy(initData.EXPIRY_DATE_TYPE)
    /**
	 * 分页参数
	 */

    ...
    $httpProvider.interceptors.push(function($q,$rootScope){
        return {
            response: function(response) {
                if (response.data.dataStatus == "dataError") {
                    alert('没有足够的数据权限');
                    history.go(-1);
                }
                return response;
    ...
    $ocLazyLoadProvider.config({
        ...
        modules: [
            ...
            },
            {
                ...
                files: ['js/plugins/summernote/angular-summernote.min.js', 'js/plugins/summernote/summernote.min.js','css/plugins/summernote/summernote.css']
            },{
                name: 'summernote',

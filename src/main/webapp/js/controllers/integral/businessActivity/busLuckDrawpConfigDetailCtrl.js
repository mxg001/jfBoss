 ...
 * 抽奖
 */
angular.module('inspinia',['uiSwitch']).controller('busLuckDrawpConfigDetailCtrl',function($scope,$http,i18nService,SweetAlert,$document,$stateParams,$state){
    ...
    $scope.userGrid={                           //配置表格
        ...
        columnDefs:[                           //表格数据
            ...
            { field: 'id',displayName:'操作',width:150,pinnedRight:true, cellTemplate:
                '<div class="lh30">'+
                    '<a target="_blank" ui-sref="management.busPrizeDetail({businessNo:grid.appScope.businessNo,awardCode:row.entity.awardCode,source:grid.appScope.source})" >详情</a> ' +
                    '<a target="_blank" ui-sref="management.blacklistBackDetail({businessNo:grid.appScope.businessNo,actCode:grid.appScope.actCode,awardCode:row.entity.awardCode,source:grid.appScope.source})" > | 黑名单</a> ' +
                '</div>'
            }
        ],

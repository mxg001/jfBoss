 ...
 * 业务组-奖项新增
 */
angular.module('inspinia',['angularFileUpload']).controller('busPrizeAddCtrl',function($scope,$http,i18nService,$state,FileUploader,$stateParams){
    ...
    $scope.addInfo={businessNo:$scope.businessNo,actCode:$stateParams.actCode,actDetCode:$stateParams.actDetCode,prize:{rewardType:"",calcType:""}};
    $scope.conList=[];
    ...
    $scope.saveInfo=function(url){
        ...
        $http.post("activityChoice/addLuckPrize",
            "info="+angular.toJson($scope.infoSub)+"&businessNo="+$scope.businessNo,
            {headers: {'Content-Type': 'application/x-www-form-urlencoded'}})
            .success(function(data){
                $scope.submitting = false;
                if(data.status){
    ...
    $scope.goBackFun=function(){
        window.open('welcome.do#/management/busLuckDrawpConfig/'+$scope.actCode+'/'+$scope.businessNo, '_self');
    };
});
 ...
 * 用户激活
 */
angular.module('inspinia',['angularFileUpload']).controller('busActivityConfigCtrl',function($scope,$http,i18nService,SweetAlert,$document,$stateParams,$state,FileUploader){
    ...
    //明细执行状态
    $scope.actDetStatusSelect = [{text:"未执行",value:"0"},{text:"执行中",value:"1"},{text:"执行完成",value:"2"}];
    $scope.actDetStatusStr=angular.toJson($scope.actDetStatusSelect);
    ...
    $scope.userGrid={                           //配置表格
        ...
        columnDefs:[                           //表格数据
            ...
            { field: 'actDetStatus',displayName:'执行状态',cellFilter:"formatDropping:" +  $scope.actDetStatusStr,width:150},
            { field: 'id',displayName:'操作',width:180,pinnedRight:true, cellTemplate:
                '<div class="lh30">'+
                    '<a target="_blank" ui-sref="management.busContentDetail({businessNo:grid.appScope.businessNo,actDetCode:row.entity.actDetCode,source:3})" >详情</a> ' +
                    '<a target="_blank" ng-show="grid.appScope.hasPermit(\'activityChoice.editActiInfo\')" ui-sref="management.busContentEdit({businessNo:grid.appScope.businessNo,actDetCode:row.entity.actDetCode})" > | 修改</a> ' +
                    '<a ng-show="grid.appScope.hasPermit(\'activityChoice.deleteActiInfo\')" ng-click="grid.appScope.deleteActiInfo(row.entity.actDetCode)" > | 删除 </a> ' +
                '</div>'
            }
        ],
    ...
    $scope.saveActivityTemp=function(url){
        ...
        $http.post("activityChoice/saveActivity",data,postCfg)
            .success(function(data){
                $scope.submitting = false;
                if(data.status){
                    $scope.notice(data.msg);
    ...
    $scope.addPrizeClick=function(){
        if($scope.addInfo.actCode!=null&&$scope.addInfo.actCode!=""){
            window.open('welcome.do#/management/busContentAdd/'+$scope.businessNo+'/'+$scope.addInfo.actCode, 'view_window');
        }else{
    ...
    $scope.deleteActiInfo=function(actDetCode){
        SweetAlert.swal({
            ...
            function (isConfirm) {
                if (isConfirm) {
                    $http.post("activityChoice/deleteActiInfo","actDetCode="+actDetCode+"&businessNo="+$scope.businessNo,
                        {headers: {'Content-Type': 'application/x-www-form-urlencoded'}})
                        .success(function(data){
                            if(data.status){
                                $scope.notice(data.msg);

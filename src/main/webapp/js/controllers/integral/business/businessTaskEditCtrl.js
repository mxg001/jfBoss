 ...
 * 业务组-任务修改
 */
angular.module('inspinia',['angularFileUpload']).controller('businessTaskEditCtrl',function($scope,$http,i18nService,$state,$stateParams,SweetAlert,FileUploader){
    ...
    $scope.addInfo={taskType:"",grantType:""};
    ...
    //获取相关券
    $scope.showRewardSelectList = false;//是否显示券选择下拉菜单
    $scope.getAllGoodsType=function(){
        if ($scope.loadImg) {
            return;
        }
        $scope.loadImg = true;
        $http.post("goodsManagement/getAllGoodsType","info="+angular.toJson($scope.rewardInfo)+"&pageNo="+
          $scope.paginationOptions.pageNo+"&pageSize="+$scope.paginationOptions.pageSize,
          {headers: {'Content-Type': 'application/x-www-form-urlencoded'}})
          .success(function(res){
              if(res.status){
                  $scope.rewardSelectList = [];
                  $scope.rewardSelectList.unshift({"value":null,"text":"请选择"});
                  if(res.data!=null && res.data.length>0){
                      for(var i=0; i<res.data.length; i++){
                          $scope.rewardSelectList.push({"value":res.data[i].goodsCode, "text":res.data[i].goodsName});
                      }
                  }
              }else{
                  $scope.notice(res.msg);
    ...
    $scope.addModal = function(){
        ...
        $scope.addInfoEntry={rewardType:"1",calcType:"1","goodsCode":null};
    ...
    $scope.rewardTypeChange=function (updateCalcType) {
        ...
        if($scope.addInfoEntry.rewardType==4 || $scope.addInfoEntry.rewardType==5 || $scope.addInfoEntry.rewardType==6){
            $scope.addInfoEntry.goodsCode = $scope.addInfoEntry.calcType=="1"?null:$scope.addInfoEntry.calcType;
            if($scope.addInfoEntry.id==null){
                $scope.calcTypeSta = false;
            }else{
                $scope.calcTypeSta = true;
                if(!updateCalcType) {
                    $scope.addInfoEntry.goodsCode = null;
                }
            }
            $scope.calcTypeChange();
            $scope.showRewardSelectList = true;
            $scope.rewardInfo.goodsType = $scope.addInfoEntry.rewardType;
            $scope.getAllGoodsType();
        }else{
            if($scope.addInfoEntry.id!=null){
                if(!updateCalcType){
    ...
    $scope.calcTypeChange=function () {
        if($scope.addInfoEntry.calcType=="1" || $scope.addInfoEntry.rewardType == 4 || $scope.addInfoEntry.rewardType == 5 || $scope.addInfoEntry.rewardType == 6){
            $scope.calcTypeSta=true;
        }else{
            $scope.calcTypeSta=false;
        }
    ...
    $scope.saveEntry=function (sta) {
        ...
        if($scope.addInfoEntry.rewardType==4 || $scope.addInfoEntry.rewardType==5 || $scope.addInfoEntry.rewardType==6){
            $scope.addInfoEntry.calcType = $scope.addInfoEntry.goodsCode;
        }
        //新增
        if(sta==1){
    ...
    $scope.initAddInfo=function(){
        ...
        }else if($scope.addInfoEntry.calcType=="2"){
    ...
    $scope.addInfoSubmit = function(){
        ...
        $http.post("businessInfoAction/editBusinessTask",data,postCfg)
            .success(function(data){
                if(data.status){
                    $scope.notice(data.msg);
                    $state.transitionTo('management.businessTask',{businessNo:$scope.businessNo},{reload:false});
                }else{
                    $scope.notice(data.msg);
                }
                $scope.submitting = false;

...
//          {field: 'status', displayName: '状态', cellFilter:"formatDropping:"+ angular.toJson($scope.status)},
	...
	$scope.busiList=[];
	//获取业务list
	$scope.getBusinessInfoList=function(){
		$http.post("businessInfoAction/getBusinessInfoListAll",
			{headers: {'Content-Type': 'application/x-www-form-urlencoded'}})
			.success(function(msg){
				if(msg.status){
					$scope.busiListBase = msg.list;//基础数据列表
				}else{
					$scope.notice("查询出错");
				}
			});
	};
	$scope.getBusinessInfoList();

	//全选,子选项全部选择
	$scope.buCheckAllChange = function(buCheckAll){
		if(buCheckAll){
			angular.forEach($scope.busiList,function(item){
	...
	//子选项去除掉，取消全选
	$scope.buCheckItemChange = function(item){
		if (item.checkedState==false) {
			$scope.newRightInfo.buCheckAll=false;
		}
	};

	//增加新的角色
	$scope.addRightModal = function(){
		$scope.busiList=angular.copy($scope.busiListBase);//操作数据
		$scope.newRightInfo = {buCheckAll:false};
	...
	$scope.editRightModal = function(entity){
		...
		$scope.initBusiListModify($scope.rightInfo)
		$("#editRightModal").modal("show");
	};
	$scope.initBusiListModify=function (rightInfo) {
		$scope.busiListModify=angular.copy($scope.busiListBase);//操作数据
		var strs=rightInfo.businessNos;
		if(strs=="-1"){
			rightInfo.buCheckAll=true;
			$scope.buCheckAllChangeModify(rightInfo.buCheckAll);
		}else{
			var strs=","+strs+",";
			angular.forEach($scope.busiListModify,function(item){
				if(strs.indexOf(","+item.businessNo+",")>-1){
					item.checkedState = true;
				}
			})
		}
	...
	//全选,子选项全部选择
	$scope.buCheckAllChangeModify = function(buCheckAll){
		if(buCheckAll){
			angular.forEach($scope.busiListModify,function(item){
				item.checkedState = true;
			})
	...
	//子选项去除掉，取消全选
	$scope.buCheckItemChangeModify = function(item){
		if (item.checkedState==false) {
			$scope.rightInfo.buCheckAll=false;
		}
	};
	//验证编码是否唯一
	...
	$scope.pottingInfo = function (addInfo,list) {
		var str="";
		var strlength=0;
		angular.forEach(list,function(item){
			if(item.checkedState){
				if(""==str){
					str=str+item.businessNo;
				}else{
					str=str+","+item.businessNo;
				}
				strlength++;
		...
		if(""==str){
			$scope.notice("业务组至少选择一个!");
			return false;
		}
		if(strlength==list.length){
			addInfo.businessNos="-1";
		}else{
			addInfo.businessNos=str;
		}
		return true;
	};
	//提交新的角色
	$scope.submitNewRight = function(){
		...
		if(!$scope.pottingInfo($scope.newRightInfo,$scope.busiList)){
			return;
		}
		$scope.submitting = true;
	...
	$scope.submitEditRight = function(){
		if(!$scope.pottingInfo($scope.rightInfo,$scope.busiListModify)){
			return;
		}
		$scope.submitting = true;

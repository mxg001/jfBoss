 ...
 * 活动内容新增
 */
angular.module('inspinia').controller('contentAddCtrl',function($scope,$http,i18nService,$state,$stateParams,SweetAlert){
    ...
    $scope.addInfo={activeType:"",awardMethod:"",actCode:$scope.actCode};
    ...
    $scope.commitInfo=function(){
        ...
        $scope.infoSub.actCode=$scope.actCode;
        $scope.infoSub.actReward=$scope.addInfoItemList;
        $http.post("activityManagement/addActiInfo",
            "info="+angular.toJson($scope.infoSub)+"&activeDesc="+encodeURI($scope.activeDescTmp),
    ...
    $scope.goBackFun=function(){
        window.open('welcome.do#/activities/activityConfig/'+$scope.actCode+'/0', '_self');
    };
});
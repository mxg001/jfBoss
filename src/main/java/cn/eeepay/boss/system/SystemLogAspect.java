...
public  class SystemLogAspect { 
	...
	public void before(JoinPoint joinPoint){
		try {
			...
			log.info("操作log日志:---->");
			log.info("请求url:"+request.getRequestURI());//请求url
			log.info("请求方法:"+joinPoint.getTarget().getClass().getName() + "." + joinPoint.getSignature().getName() + "()");//请求方法
			log.info("请求参数:"+getRequestParams(joinPoint));//请求参数
			final UserLoginInfo principal = (UserLoginInfo) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
			log.info("操作人:"+principal.getUsername());//操作人
		}  catch (Exception e) {
			//记录本地异常日志
			e.printStackTrace();

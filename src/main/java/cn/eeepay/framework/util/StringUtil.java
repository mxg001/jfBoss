...
//	                int temp = random.nextInt(2) % 2 == 0 ? 65 : 97;  
	 ...
	 * @param list 字符list
	 * @param spacer 分隔符
	 * @param  symbol 是否需要单引号
	 * @return  如1,2,3,4
	 */
	public static String listToString(List<String> list,String spacer,boolean symbol) {
		if(list!=null&&list.size()>0){
			StringBuffer sb=new StringBuffer();
			for(String item:list){
				if(symbol){
					sb.append("'"+item+"'"+spacer);
				}else{
					sb.append(item+spacer);
				}
	...
	/**
	 *
	 * @param businessNos 权限集合 1,2,3,4
	 * @param businessNo 单个
	 * @return true 不包含，fas
	 */
	public static boolean checkBusinessNo(String businessNos,String businessNo){
		if(StringUtils.isNotBlank(businessNos)&&StringUtils.isNotBlank(businessNo)){
			if("-1".equals(businessNos)){
				return true;
			}else{
				if((","+businessNos+",").indexOf(",'"+businessNo+"',")>-1){
					return true;
				}
			}
		}
	...
	public static String sensitiveInformationHandle(String str,int state){
		if(StringUtils.isNotBlank(str)){
			if(0==state){
				...
				return str.replaceAll("(\\d{2})\\d{14}(\\w{2})","$1********$2");
			}else if(2==state){

...
public interface SysDictDao {
	...
	@Select("select * from vip_sys_dict where status=1 and sys_key=#{key}")
	@ResultType(SysDict.class)
	...
	@Select("SELECT sd.id,sd.sys_key,sd.sys_name,sd.sys_value,ag.group_no,ag.relation_type FROM `vip_sys_dict` sd"
	...
	@Select("select sd.* from vip_sys_dict sd "
			+ " where sd.status=1 and sd.parent_id=#{key} and sd.sys_key=#{key}")
	@ResultType(SysDict.class)
	List<SysDict> selectListByKey(@Param("key")String key);
	
	@Insert("insert into vip_sys_dict (sys_key,sys_name,sys_value,parent_id,order_no,status,remark) values "
	...
	@Update("update vip_sys_dict set sys_key=#{info.sysKey},sys_name=#{info.sysName},sys_value=#{info.sysValue},"
			+ "parent_id=#{info.parentId},order_no=#{info.orderNo},status=#{info.status},remark=#{info.remark} "
			+ "where id=#{info.id}")
	int update(@Param("info")SysDict info);

	@Delete("delete from vip_sys_dict where id=#{id}")
	int delete(@Param("id")Integer id);

	@Select("SELECT s.*,p.sys_value `type` FROM vip_sys_dict p LEFT JOIN vip_sys_dict s  ON p.sys_key=s.parent_id WHERE s.status=1 and p.parent_id='BOSS_INIT' ORDER BY s.sys_key,s.order_no")
	@ResultType(SysDict.class)
	List<SysDict> selectAllDict();
	
	@Select("select * from vip_sys_dict where status=1 and sys_key='INITIAL_PWD' limit 1")
	@ResultType(SysDict.class)
	SysDict selectRestPwd();
	
	@Select("select * from vip_sys_dict where sys_value=#{serviceType} and parent_id=#{acqServiceType}")
	@ResultType(SysDict.class)
	SysDict selectExistServiceLink(@Param("serviceType")String serviceType, @Param("acqServiceType")String acqServiceType);
	
	@Select("select sys_key,sys_name,sys_value from vip_sys_dict where sys_key=#{key} and parent_id=#{key}")
	@ResultType(SysDict.class)
	List<SysDict> selectByKey(@Param("key")String key);

	@Update("update vip_sys_dict set sys_value=#{sysValue} where parent_id='BOSS_UNIQUE' and sys_key=#{sysKey}")
	int updateSysValue(SysDict sysDict);
	
	@Update("update vip_sys_dict set sys_value=#{sysValue} where parent_id='BOSS_UNIQUE' and sys_key=#{sysKey}")
	int updateValueByKey(@Param("sysValue")String sysValue, @Param("sysKey")String sysKey);
	
	@Select("select sys_value from vip_sys_dict where sys_key=#{sysKey}")
	String getValueByKey(@Param("sysKey")String sysKey);
	
	@Select("select sys_value from vip_sys_dict where parent_id=#{parentId} and status=1")
	 ...
	 * @return
	 */
	@Select("select sd.id,sd.sys_value,sd.sys_name from vip_sys_dict sd "

...
public interface BossOperLogDao {
	...
	//添加
    @Insert("insert into vip_boss_oper_log(user_id,user_name,oper_code,request_method,request_params,"
	...
	@Select("SELECT bol.*, CONCAT( IFNULL( concat(ppm.menu_name, '-'), '' ), pm.menu_name,"
		+"'-', m.menu_name ) menu_name FROM vip_boss_oper_log bol LEFT JOIN vip_boss_sys_menu m ON bol.oper_code = m.menu_code "
        +" LEFT JOIN vip_boss_sys_menu pm ON m.parent_id = pm.id"
        +" LEFT JOIN vip_boss_sys_menu ppm ON pm.parent_id = ppm.id"
        +" WHERE bol.id=#{id}")
	@Results(value ={
			@Result(id=true, property="id",column="id",javaType=Long.class,jdbcType=JdbcType.INTEGER),
			@Result(property="user_id",column="user_id",javaType=Integer.class,jdbcType=JdbcType.INTEGER),
	...
	public class SqlProvider{
		...
		+"'-', m.menu_name ) menu_name FROM vip_boss_oper_log bol LEFT JOIN vip_boss_sys_menu m ON bol.oper_code = m.menu_code "
        +" LEFT JOIN vip_boss_sys_menu pm ON m.parent_id = pm.id"
        +" LEFT JOIN vip_boss_sys_menu ppm ON pm.parent_id = ppm.id where 1=1 ");

...
public interface MenuDao {
	...
	@Select("select id,parent_id,menu_code,menu_name,menu_url,rigth_code,menu_level,"
			+ "menu_type,order_no,log_flag from vip_boss_sys_menu where menu_type='menu' "
	...
	@Select("select id,parent_id,menu_code,menu_name,menu_url,rigth_code,menu_level,"
			+ "menu_type,order_no from vip_boss_sys_menu where menu_type='menu' "
			+ "and menu_level=0 order by (order_no+0) ")
	@ResultMap("cn.eeepay.framework.dao.SysMenuMapper.OneToManyResultMap")
	List<SysMenu> getSysMenuAndChildren();

	@Select("select * from vip_boss_sys_menu")
	@ResultType(MenuInfo.class)
	List<MenuInfo> getAllMenu();
	
	@Select("select * from vip_boss_sys_menu where menu_type='page'")
	@ResultType(MenuInfo.class)
	...
	@Select("select r.* from vip_boss_shiro_right r,vip_boss_right_menu rm where rm.menu_id=#{id} and r.id=rm.right_id")
	 ...
	 * @return
	 */
	@Select("select m.id from vip_boss_sys_menu m join vip_boss_right_menu rm on m.id=rm.menu_id join vip_boss_shiro_right r on rm.right_id = r.id and r.right_code in (${permits})")
	@ResultType(Integer.class)
	List<Integer> getMenuIdsByPermits(@Param("permits")String permits);

	@Select("select * from vip_boss_sys_menu where id=#{parentId}")
	@ResultType(MenuInfo.class)
	MenuInfo getParent(Integer parentId);

	@Insert("insert into vip_boss_sys_menu(parent_id,menu_code,menu_name,menu_url,menu_level,menu_type,order_no) values "
	...
	@Update("UPDATE vip_boss_sys_menu set parent_id=#{menu.parentId},menu_code=#{menu.menuCode},menu_name=#{menu.menuName},menu_url=#{menu.menuUrl}"
	...
	@Delete("DELETE FROM vip_boss_sys_menu WHERE  id=#{menuId}")
	...
	@Select("select * from vip_boss_sys_menu where menu_type='page' and menu_level=2 and parent_id=#{menuId} order by (order_no+0)")
	@ResultType(MenuInfo.class)
	List<MenuInfo> findMenuPageList(Integer menuId);

	@Select("select m1.menu_code from vip_boss_sys_menu m1 left join vip_boss_sys_menu m2 on m1.menu_url=m2.menu_url where m2.menu_code=#{menuCode}")
	@ResultType(String.class)
	List<String> getMenuCodeByCode(@Param("menuCode")String menuCode);

	@Select("select m.id from vip_boss_sys_menu m where m.menu_code=#{menuCode}")
	@ResultType(MenuInfo.class)
	MenuInfo checkMenuCode(String menuCode);

	@Update("update vip_boss_sys_menu m set m.log_flag=#{logFlag} where m.id=#{id}")
	int updateStatus(MenuInfo info);
	
	@Select("select m.* from vip_boss_sys_menu m where m.parent_id=#{parentId} order by m.order_no;")
	@ResultType(MenuInfo.class)

...
public interface GrantDao {
	...
	public static final String GRANT_USER_RIGHT="INSERT INTO vip_boss_user_right(user_id,right_id) values ";
	public static final String GRANT_RIGHT_USER="INSERT INTO vip_boss_user_right(right_id,user_id) values ";
	...
	public static final String GRANT_RIGHT_MENU="INSERT INTO vip_boss_right_menu(right_id,menu_id) values ";
	public static final String GRANT_MENU_RIGHT="INSERT INTO vip_boss_right_menu(menu_id,right_id) values ";
	public static final String GRANT_USER_MENU="INSERT INTO vip_boss_user_menu(user_id,menu_id) values ";
	...
	@Delete("DELETE FROM vip_boss_user_right where user_id=#{userId} and right_id=#{rightId}")
	int delUserRight(@Param("userId") Integer userId, @Param("rightId") Integer rightId);
	@Insert("insert into vip_boss_user_right(user_id,right_id) values(#{userId},#{rightId})")
	int insertUserRight(@Param("userId") Integer userId, @Param("rightId") Integer rightId);

	@Delete("delete from vip_boss_user_menu where user_id=#{userId}")
	...
	@Delete("DELETE FROM vip_boss_right_menu where right_id=#{rightId}")
	void deleteRightMenuByRightId(Integer rightId);
	@Delete("DELETE FROM vip_boss_right_menu where menu_id=#{menuId}")
	void deleteRightMenuByMenuId(Integer menuId);
	@Delete("DELETE FROM vip_boss_right_menu where right_id=#{rightId} and menu_id=#{menuId}")
	int delRightMenu(@Param("rightId") Integer rightId, @Param("menuId") Integer menuId);
	@Insert("insert into vip_boss_right_menu(right_id,menu_id) values(#{rightId},#{menu_id})")
	...
	@Select("SELECT sr.* FROM (SELECT right_id FROM vip_boss_user_right WHERE user_id=#{userId} UNION"
			+ " SELECT rr.right_id FROM boss_user_role ur,boss_role_right rr WHERE ur.role_id=rr.role_id "
			+ " AND user_id=#{userId}) urc LEFT JOIN vip_boss_shiro_right sr  ON urc.right_id=sr.id")
	@ResultType(RightInfo.class)
	public List<RightInfo> getUserAllRight(Integer userId);
	
	@Select("SELECT sr.* FROM (SELECT menu_id FROM vip_boss_user_menu WHERE user_id=#{userId} UNION"
			+ " SELECT rr.menu_id FROM vip_boss_user_right ur,vip_boss_right_menu rr WHERE ur.right_id=rr.right_id "
			+ " AND user_id=#{userId}) urc LEFT JOIN vip_boss_sys_menu sr  ON urc.menu_id=sr.id")

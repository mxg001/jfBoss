...
public interface UserDao {
	...
	@Insert("insert into vip_boss_shiro_user(user_name,password,real_name,tel_no,email,status,create_operator) "
	...
	@Select("select 1 from vip_boss_shiro_user where user_name=#{userName} limit 1")
	@ResultType(Integer.class)
	Integer checkNameUnique(@Param("userName")String userName);

	@Update("update vip_boss_shiro_user set real_name=#{user.realName},tel_no=#{user.telNo},email=#{user.email},status=#{user.status} "
			+ " where id=#{user.id}")
	int updateUser(@Param("user")UserInfo user);
	
	@Update("update vip_boss_shiro_user set tel_no=#{user.newTelNo} "
			+ " where tel_no=#{telNo}")
	int updateUsertelNo(@Param("telNo")String telNo,@Param("newTelNo")String newTelNo);

	@Update("update vip_boss_shiro_user set password=#{user.password},update_pwd_time=#{user.updatePwdTime} where id=#{user.id}")
	...
	@Select("select * from vip_boss_shiro_user")
	@ResultType(UserInfo.class)
	List<UserInfo> findUserBox();
	
	@Select("select * from vip_boss_shiro_user where id=#{id}")
	@ResultType(UserInfo.class)
	...
	@Delete("DELETE FROM vip_boss_user_right WHERE user_id=#{userId}")
	int delUserRights(Integer userId);
	@Delete("DELETE FROM vip_boss_shiro_user WHERE id=#{userId}")
	...
	@Select("select r.* from vip_boss_shiro_right r , vip_boss_user_right ur where r.id=ur.right_id and ur.user_id=#{id}")
	@ResultType(RightInfo.class)
	List<RightInfo> getRightsByUser(@Param("id")Integer id);
	
	@Select("select id,user_name,real_name,status from vip_boss_shiro_user")
	@ResultType(UserInfo.class)
	List<UserInfo> getAllUsers();
	
	@Select("select m.id,m.menu_name,m.menu_url,m.menu_level,m.menu_type from vip_boss_sys_menu m,vip_boss_user_menu um where um.user_id=#{id} and m.id=um.menu_id")
	@ResultType(MenuInfo.class)
	List<MenuInfo> getMenuByUser(Integer id);

	@Select("select * from vip_boss_shiro_user where status=1 and user_name=#{userName}")
	@ResultType(UserInfo.class)
	UserInfo selectUserByUserName(@Param("userName")String userName);
	
	@Select("SELECT u.* FROM vip_boss_shiro_user u, vip_boss_user_right ur, vip_boss_right_menu rm, vip_boss_sys_menu m"
			+" WHERE"
			+" u.id = ur.user_id"
	...
	@Select(
		"select t1.user_name,t3.business_nos " +
				" from vip_boss_shiro_user t1 " +
				"  INNER JOIN vip_boss_user_right t2 ON t2.user_id=t1.id " +
				"  INNER JOIN vip_boss_shiro_right t3 ON t3.id=t2.right_id " +
				" where t1.user_name=#{userName} "
	)
    List<UserBusinessNo> getUserBusinessNoList(@Param("userName")String userName);
    ...
    public class SqlProvider{
		...
		public String getUserlimitSql(Map<String, Object> param){
			...
			sb.append(" From vip_boss_shiro_user ");

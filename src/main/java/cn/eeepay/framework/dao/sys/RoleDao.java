...
public interface RoleDao {
	...
	@Select("SELECT u.* FROM boss_user_role ur,vip_boss_shiro_user u WHERE u.id=ur.user_id AND ur.role_id=#{id}")
	@ResultType(UserInfo.class)
	List<UserInfo> getUsersByRole(Integer id);
	
	@Select("SELECT r.* FROM boss_role_right rr,vip_boss_shiro_right r WHERE r.id=rr.right_id AND rr.role_id=#{id}")
